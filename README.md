...
- When empty values are provided for title or date, then the error message **Please fill both the title and date** should be displayed
- When non-empty values are provided for title and date and the **Add** button is clicked,
  ...
  - The new appointment is saved to localStorage
  - The value inside the input elements for title and date should be updated to their initial values
- When the `star` button in an appointment item is clicked,
  - The `star` image in the button of an appointment item should be updated with the `filled star` image
  - The appointment item should be starred.
- When the `filled star` button in a starred appointment item is clicked,
  - The `filled star` image in the button of a starred appointment item should be updated with the `star` image
  - The appointment item should be unstarred.
- The appointments list along with their starred/unstarred status should persist even after page reload. Use Local Storage for persistence.
